  ...
  module ServiceBoundary
    class Validator
      ...
      def call(package_set, configuration)
        ...
        package_manifests_settings_for(configuration, 'enforce_service_boundary').each do |config, setting|
          next if setting.nil?
          ...
          results << Result.new(
            ok: false,
            error_value: "\tInvalid 'enforce_service_boundary' option: #{setting.inspect} in #{config.inspect}"
          )
        end
